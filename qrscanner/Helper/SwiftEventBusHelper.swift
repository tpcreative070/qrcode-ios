...
import Foundation
open class SwiftEventBusHelper {
    
    struct Static {
        static let instance = SwiftEventBusHelper()
        static let queue = DispatchQueue(label: "ecpay.vn.SwiftEventBus", attributes: [])
    }
    ...
    open class func onMainThread(_ target: AnyObject, name: String, sender: Any? = nil, handler: @escaping ((Notification?) -> Void)) -> NSObjectProtocol {
        return SwiftEventBusHelper.on(target, name: name, sender: sender, queue: OperationQueue.main, handler: handler)
    }
    ...
    open class func onBackgroundThread(_ target: AnyObject, name: String, sender: Any? = nil, handler: @escaping ((Notification?) -> Void)) -> NSObjectProtocol {
        return SwiftEventBusHelper.on(target, name: name, sender: sender, queue: OperationQueue(), handler: handler)
    }
