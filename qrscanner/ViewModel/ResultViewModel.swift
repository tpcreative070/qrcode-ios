...
class ResultViewModel : ResultViewModelDelegate {
    ...
    func doInsert(mValue : GenerateEntityModel){
        let result = SQLHelper.insertedScanner(data: GenerateEntityModel(createdDateTime: Date().millisecondsSince1970, typeCode: typeCode ?? "", content: mValue.content!, isHistory: false, isSave: true, updatedDateTime: Date().millisecondsSince1970, bookMark: false, transactionID: "", isCode: ""))
        if result {
