...
import Foundation
import UIKit
@IBDesignable public class ICStepper: UIControl {
    ...
    lazy var leftButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(ICStepper.leftButtonTouchDown), for: .touchDown)
        button.addTarget(self, action: #selector(ICStepper.buttonTouchUp), for: .touchUpInside)
        button.addTarget(self, action: #selector(ICStepper.buttonTouchUp), for: .touchUpOutside)
        button.addTarget(self, action: #selector(ICStepper.buttonTouchUp), for: .touchCancel)
        button.titleLabel?.font = AppFonts.moderateScale(fontName: AppFonts.SFranciscoRegular, size: AppFonts.ITEM_LABEL_FONT_SIZE)
        return button
    }()
    ...
    lazy var rightButton: UIButton = {
        ...
        button.backgroundColor = self.buttonsBackgroundColor
        button.titleLabel?.font = self.buttonsFont
        button.addTarget(self, action: #selector(ICStepper.rightButtonTouchDown), for: .touchDown)
        button.addTarget(self, action: #selector(ICStepper.buttonTouchUp), for: .touchUpInside)
        button.addTarget(self, action: #selector(ICStepper.buttonTouchUp), for: .touchUpOutside)
        button.addTarget(self, action: #selector(ICStepper.buttonTouchUp), for: .touchCancel)
        button.titleLabel?.font = AppFonts.moderateScale(fontName: AppFonts.SFranciscoRegular, size: AppFonts.ITEM_LABEL_FONT_SIZE)
    ...
    lazy var label: UILabel = {
        ...
        let panRecognizer = UIPanGestureRecognizer(target: self, action: #selector(ICStepper.handlePan))
        panRecognizer.maximumNumberOfTouches = 1
        label.addGestureRecognizer(panRecognizer)
        label.font = AppFonts.moderateScale(fontName: AppFonts.SFranciscoRegular, size: AppFonts.ITEM_LABEL_FONT_SIZE)
        return label
    }()
    ...
    fileprivate func setup() {
        ...
        setupNumberFormatter()
        NotificationCenter.default.addObserver(self, selector: #selector(ICStepper.reset), name: UIApplication.willResignActiveNotification, object: nil)
    }
...
extension ICStepper {
    ...
    func scheduleTimer() {
        timer = Timer.scheduledTimer(timeInterval: timerInterval, target: self, selector: #selector(ICStepper.handleTimerFire), userInfo: nil, repeats: true)
    }
