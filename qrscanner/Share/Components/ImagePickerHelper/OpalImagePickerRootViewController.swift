...
open class OpalImagePickerRootViewController: UIViewController {
    ...
    private func setupTabs() {
        ...
        NSLayoutConstraint.activate([
            toolbar.constraintEqualTo(with: view.safeAreaLayoutGuide, receiverAttribute: .top, otherAttribute: .bottom),
    ...
    open override func viewDidLoad() {
        ...
        navigationItem.title = configuration?.navigationTitle ?? NSLocalizedString(LanguageHelper.getTranslationByKey(LanguageKey.Gallery) ?? "Gallery", comment: "")
        
        let cancelButtonTitle = configuration?.cancelButtonTitle ?? NSLocalizedString(LanguageHelper.getTranslationByKey(LanguageKey.Cancel) ?? "Cancel", comment: "")
        ...
        let doneButtonTitle = configuration?.doneButtonTitle ?? NSLocalizedString(LanguageHelper.getTranslationByKey(LanguageKey.Done) ?? "Done", comment: "")
    ...
    @objc func doneTapped() {
        ...
        dismiss(animated: true) { [weak self] in
            if let mImages = self?.savedImages {
                 self?.delegate?.imagePicker?(imagePicker, didFinishPickingImages: mImages)
            }
        }
...
extension OpalImagePickerRootViewController: UICollectionViewDelegate {
    ...
    public func collectionView(_ collectionView: UICollectionView, shouldSelectItemAt indexPath: IndexPath) -> Bool {
        ...
        if maximumSelectionsAllowed <= collectionViewItems + externalCollectionViewItems {
            ...
            let mMessage = String(format: LanguageHelper.getTranslationByKey(LanguageKey.YouCouldNotSelectMoreThan) ?? "", arguments: [maximumSelectionsAllowed.description])
            let message = configuration?.maximumSelectionsAllowedMessage ?? mMessage
