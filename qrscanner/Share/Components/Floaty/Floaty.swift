...
open class Floaty: UIView {
   ...
   */
  @objc @IBInspectable
  open var buttonColor: UIColor = UIColor(red: 75/255.0, green: 136/255.0, blue: 216/255.0, alpha: 1)
   ...
   */
  @objc @IBInspectable
  open var buttonImage: UIImageView? = nil {
    didSet {
      self.setNeedsDisplay()
    }
  }
...
//    plusLayer.lineWidth = 2.0
  //  plusLayer.path = plusBezierPath().cgPath
    //layer.addSublayer(imgPlayer)
    
    let myLayer = CALayer()
    let myImage = UIImage(named: AppImages.IC_MENU)?.cgImage
    myLayer.frame = CGRect(x: circleLayer.frame.width/2 - (size/4), y: circleLayer.frame.height/2 - (size/4), width: 1.5*size/3, height: 1.5*size/3)
  ...
  fileprivate func setButtonImage() {
    ...
    buttonImageView.image = UIImage(named: AppImages.IC_MENU)
